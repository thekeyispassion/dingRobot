...
基于 OpenClaw 框架的智能会议室预约系统，以钉钉群机器人形态运行。数据以本地 markdown 文件为主，钉钉 AI 表格为只读镜像（用于在线查看和链接分享）。
...
├── data/                        # 数据文件（主）——git 跟踪，部署即用
│   ├── 会议室信息.md             #   会议室表（markdown 表格）
│   ├── 预约记录.md               #   预约流水（markdown 表格）
│   └── 管理员.md                 #   管理员表（markdown 表格）
│   └── 操作日志.md               #   操作日志——所有写操作自动记录
├── meeting_room/                # Skill: 会议室预约（7 个操作）
│   └── SKILL.md                 #   读写 data/ markdown 文件
├── mirror-sync/                 # Skill: 镜像同步——日志驱动增量同步
│   ├── SKILL.md                 #   同步流程 + 触发规则
│   └── 钉钉AI表格-配置.md        #   钉钉镜像字段映射 + 翻译规则
├── room_manager/                # Skill: 会议室管理（5 个操作）
│   └── SKILL.md                 #   三级权限模型 + 协商确认流程
├── manage-admin/                # Skill: 管理员账号管理
│   └── SKILL.md                 #   添加/删除/查看管理员 + 双重匹配 + 通讯录 MCP
...
- 钉钉通讯录 MCP 已配置（在 [AI Hub](https://aihub.dingtalk.com/#/mcp) 找到并配置）——用于管理员管理时解析用户名→userId
- 如需钉钉 AI 表格镜像（在线查看/分享链接），额外配置钉钉 AI 表格 MCP
